...
async def gemini_generate_buttons(user_request: str):
    """يستدعي Gemini عبر HTTP مع تجربة نماذج بديلة تلقائياً عند الفشل."""
    if not GEMINI_API_KEY:
        ...
        for model in GEMINI_MODELS:
            ...
            try:
                ...
                if resp.status_code in (429, 503):
                    logging.warning(f"Gemini model {model} unavailable ({resp.status_code}), trying next...")
                ...
                logging.error(f"Gemini HTTP error ({model}): {e.response.text}")
                if code in (401, 403):
                    return None, "❌ مفتاح Gemini API غير صالح أو لا يملك صلاحية."
                last_error = code
                continue
            except Exception as e:
                logging.error(f"Gemini error ({model}): {e}")
                last_error = str(e)
                continue
    if last_error in (429,):
        return None, "⚠️ تجاوزت الحصة المجانية لجميع النماذج. انتظر قليلاً ثم حاول مجدداً."
    if last_error in (503,):
        return None, "⚠️ خوادم Gemini مشغولة الآن. حاول مجدداً بعد لحظات."
    return None, f"❌ تعذّر الاتصال بـ Gemini. حاول مرة أخرى."
